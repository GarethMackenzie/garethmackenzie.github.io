...
for (const width of widths) {
  ...
  for (const route of routes) {
    await page.goto(`${baseUrl}${route}?live-audit=7fc112d`, { waitUntil: 'domcontentloaded' });
...
await page.goto(`${baseUrl}/?menu-audit=7fc112d`, { waitUntil: 'domcontentloaded' });
...
await page.goto(`${baseUrl}/?amazon-audit=7fc112d`, { waitUntil: 'domcontentloaded' });
await page.evaluate(() => {
  document.addEventListener('click', event => {
    if (event.target.closest('a[data-amazon-cta]')) event.preventDefault();
  }, true);
});
await page.locator('a[data-amazon-cta]:visible').first().click();
...
await page.goto(`${baseUrl}/contact/?form-audit=7fc112d`, { waitUntil: 'domcontentloaded' });

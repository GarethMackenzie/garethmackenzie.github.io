...
COVER_SRCSET = (
    '/assets/book-cover-480.webp 480w, '
    '/assets/book-cover-640.webp 640w, '
    '/assets/book-cover-800.webp 800w, '
    '/assets/book-cover-1200.webp 1200w'
)
HOME_COVER_SIZES = '(max-width:480px) 72vw, (max-width:1024px) 340px, 390px'
...
if preload_count == 0:
    raise RuntimeError('Homepage cover preload was not found')

# Keep the source picture in sync with the responsive preload.
home_text = re.sub(
    r'srcset="/assets/book-cover-480\.webp 480w, /assets/book-cover-(?:640\.webp 640w, )?800\.webp 800w, /assets/book-cover-1200\.webp 1200w"\s+sizes="[^"]+"',
    f'srcset="{COVER_SRCSET}" sizes="{HOME_COVER_SIZES}"',
    home_text,
    count=1,
)
...
built_text = re.sub(
    r'srcset="/assets/book-cover-480\.webp 480w, /assets/book-cover-(?:640\.webp 640w, )?800\.webp 800w, /assets/book-cover-1200\.webp 1200w"',
    f'srcset="{COVER_SRCSET}"',
    built_text,
    count=1,
)
...
# Budgets keep every generated rendition bounded while selecting the highest
# quality candidate that fits. The master JPEG remains unchanged.
size_budgets = {
    480: 68_000,
    640: 125_000,
    800: 200_000,
...
with Image.open(source_path) as source:
    ...
    for width, target in size_budgets.items():
